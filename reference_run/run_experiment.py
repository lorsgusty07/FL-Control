...
def run(seed,policy):
    ...
    # data-derived and fixed before comparing policies: maximum of the six observed calibration workloads
    deadline=float(np.quantile(cal.epochs,1.00,method='higher'))
...
def main():
    ...
    config={'dataset':'WISDM v1.1 transformed real-window trace subset','users':[13,17,20,27,29,33],'train_windows_per_user':8,'test_windows_per_user':4,'seeds':SEEDS,'K':K,'rounds':ROUNDS,'policies':POLICIES,'model':'MLP 7-16-6','local_service':'train until 20% local-loss reduction or 25 epochs','SLO':'parallel-equivalent local epochs <= seed-specific maximum of the six real calibration costs','no_synthetic_resources':True}
